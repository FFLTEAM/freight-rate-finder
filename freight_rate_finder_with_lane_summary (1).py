...
# Generate 1000 random embedded records with date and carrier fields
cities = [
...
if not origin_coords and not destination_coords:
    ...
    display_cols = ['rate', 'rate_per_mile', 'equipment_type', 'carrier_name', 'origin_city', 'origin_state', 'origin_country', 'destination_city', 'destination_state', 'destination_country', 'distance_miles', 'service_type', 'rate_date']
    st.dataframe(df_filtered[display_cols])

    if not df_filtered.empty:
        st.subheader("Lane Pricing Statistics")
        lanes = df_filtered.groupby(['origin_city', 'destination_city'])['rate'].agg(['min', 'mean', 'max']).reset_index()
        lanes.columns = ['Origin', 'Destination', 'Lowest Rate', 'Average Rate', 'Highest Rate']
        st.dataframe(lanes)
