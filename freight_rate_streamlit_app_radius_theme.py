...
# Theme and layout
st.set_page_config(
    page_title="Freight Rate Finder",
    layout="wide",
    initial_sidebar_state="expanded"
)

# Load and display logo
logo = Image.open("1920x1080FFL Logo.jpg")
...
if uploaded_file is not None:
    try:
        ...
        if not required_columns.issubset(df.columns):
            ...
            # Radius input (placeholder, would require geo data)
            st.sidebar.markdown("📍 **Radius Filter**")
            radius = st.sidebar.slider("Max Radius from Origin (miles)", 0, 150, 50)
