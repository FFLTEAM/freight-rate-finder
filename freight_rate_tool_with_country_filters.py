...
if uploaded_file is not None:
    try:
        ...
        # Mandatory country filters
        st.sidebar.markdown("🌍 **Select Countries**")

        origin_countries = sorted(df["Origin Country"].dropna().unique())
        selected_origin_country = st.sidebar.selectbox("Origin Country (required)", origin_countries)

        destination_countries = sorted(df[df["Origin Country"] == selected_origin_country]["Destination Country"].dropna().unique())
        selected_destination_country = st.sidebar.selectbox("Destination Country (required)", destination_countries)

        # Filter by selected countries
        df = df[
            (df["Origin Country"] == selected_origin_country) &
            (df["Destination Country"] == selected_destination_country)
        ]

        # Now mode type filter
        ...
        if filter_mode == "State Pairing":
            ...
            destination_states = sorted(df[df["Origin State"] == selected_origin_state]["Destination State"].dropna().unique())
            selected_destination_state = col2.selectbox("🗺️ Destination State", destination_states)
